***********************************************************************************************************************************************************************************;
*Program: 34.SAS_baBICNumPredCstatTest_SimSce3Corr                                                                                                                                 ;                                                               
*Purpose: Compute summary statistics for Number of predictors and C-statistic for baBIC method in Scenario 3 simulated testing correlated data                                     ;                                     
...
%let trainmod=outdata.baBICsim500corrSce3;
...
/*
======MONITORING: 2020-10-05, 16:10======
======MONITORING: 2020-10-05, 16:21======
*/

proc sort data=ctablesim out=outdata.sim&S.baBICtestfull_c; by sim; run;

PROC EXPORT DATA=outdata.sim&S.baBICtestfull_c
            OUTFILE= "path\sim&S.baBICtestfull_c.csv" 
            DBMS=CSV REPLACE;
     PUTNAMES=YES;
RUN;

*Summary stats of baBIC training simulated models on baBIC testing simulated data;
proc means data=outdata.sim&S.baBICtestfull_c stackodsoutput n mean clm stderr maxdec=4; 
 var C_adl C_iadl C_walk C_death C_avg;
 ods output summary=outdata.sim&S.baBICtestfull_cmeans(rename=(Mean=Mean_Sim_baBICtesting));
proc sort; by variable; run;

PROC EXPORT DATA= outdata.sim&S.baBICtestfull_cmeans
            OUTFILE= "path\sim&S.baBICtestfull_cmeans.csv" 
            DBMS=CSV REPLACE;
     PUTNAMES=YES;
RUN;
