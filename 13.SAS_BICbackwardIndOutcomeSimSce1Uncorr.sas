***********************************************************************************************************************************************************************************;
*Program: 13.SAS_BICbackwardIndOutcomeSimSce1Uncorr                                                                                                                                ;                                                               
*Purpose: BIC backward elimination for individual outcomes for Scenario 1 simulated training uncorrelated data. Obtain Union model for each simulation                             ;                                     
...
data savedata.BICnewsim&S.uncorrBy (drop=delims i variable allvars);
  set BICsim;
...
run;
/*proc print data=savedata.BICnewsim&S.uncorrBy (obs=3); var jointvars; run;*/

*Summary stats of simulated models by outcome;
proc means data=savedata.BICnewsim&S.uncorrBy stackodsoutput n mean clm stderr maxdec=4; 
 ...
 ods output summary=savedata.BICnewsim&S.uncorrBygralstats(rename=(Mean=MeanSimulationByOutcome));
proc sort; by variable; run;

PROC EXPORT DATA= savedata.BICnewsim&S.uncorrBygralstats
            OUTFILE= "path\BICnewsim&S.uncorrBygralstats.csv" 
            DBMS=CSV REPLACE;
     PUTNAMES=YES;
RUN;
