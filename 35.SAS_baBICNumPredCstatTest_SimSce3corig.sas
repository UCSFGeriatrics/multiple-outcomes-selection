***********************************************************************************************************************************************************************************;
*Program: 35.SAS_baBICNumPredCstatTest_SimSce3corig                                                                                                                                ;                                                               
*Purpose: Compute summary statistics for number of predictors and C-statistic for baBIC method in simulations of Scenario 3 with case-study censoring and testing data             ;                                     
...
%let trainmodels=outtrain.bicsimbabic_Sce3corig;
...
proc means data=&trainmodels stackodsoutput n mean clm stderr p50 p25 p75 maxdec=4; 
 ...
 ods output summary=outtrain.bicsimbabic_Sce3corig_stats;
run;

PROC EXPORT DATA= outtrain.bicsimbabic_Sce3corig_stats
            OUTFILE= "path\bicsimbabic_Sce3corig_stats.csv" 
            DBMS=CSV REPLACE;
     PUTNAMES=YES;
RUN;
...
%macro cstat;
 ...
 /*For each simulated dataset define VARNAME as the variables selected in baBIC model of training data*/
  ...
  proc append base=c_baBICSce3corig data=ctable force; run;
...
proc sort data=c_baBICSce3corig out=outtest.c_baBICSce3corig; by sim; run;

*Summary stats of baBIC training simulated models on baBIC testing simulated data;
proc means data=outtest.c_baBICSce3corig stackodsoutput n mean clm stderr maxdec=4; 
 var C_adl C_iadl C_walk C_death C_avg;
 ods output summary=outtest.c_baBICSce3corig_means (rename=(Mean=Mean_Sim_baBICtesting));
run;

PROC EXPORT DATA= outtest.c_baBICSce3corig_means
            OUTFILE= "path\c_baBICSce3corig_means.csv" 
            DBMS=CSV REPLACE;
     PUTNAMES=YES;
RUN;
...
proc stdize data=outtest.c_baBICsce3corig PctlMtd=ORD_STAT outstat=outtest.c_baBICsce3corig_percentiles pctlpts=2.5, 97.5;
...
data outtest.c_baBICsce3corig_percentiles;
 set outtest.c_baBICsce3corig_percentiles;
 where _type_ =: 'P';
...
proc print data=outtest.c_baBICsce3corig_percentiles noobs; run;
