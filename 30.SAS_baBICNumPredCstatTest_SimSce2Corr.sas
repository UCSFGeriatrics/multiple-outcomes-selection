***********************************************************************************************************************************************************************************;
*Program: 30.SAS_baBICNumPredCstatTest_SimSce2Corr                                                                                                                                 ;                                                               
*Purpose: Compute summary statistics for Number of predictors and C-statistic for baBIC method in Scenario 2 simulated testing correlated data                                     ;                                     
...
%let trainmod=outdata.baBICsim500corrSce2;
...
proc sort data=ctablesim out=outdata.sim&S.baBICtestby_c; by sim; run;

PROC EXPORT DATA=outdata.sim&S.baBICtestby_c
            OUTFILE= "path\sim&S.baBICtestby_c.csv" 
            DBMS=CSV REPLACE;
     PUTNAMES=YES;
RUN;

*Summary stats of baBIC training simulated models on baBIC testing simulated data;
proc means data=outdata.sim&S.baBICtestby_c stackodsoutput n mean clm stderr maxdec=4; 
 var C_adl C_iadl C_walk C_death C_avg;
 ods output summary=outdata.sim&S.baBICtestby_cmeans(rename=(Mean=Mean_Sim_baBICtesting));
proc sort; by variable; run;

PROC EXPORT DATA= outdata.sim&S.baBICtestby_cmeans
            OUTFILE= "path\sim&S.baBICtestby_cmeans.csv" 
            DBMS=CSV REPLACE;
     PUTNAMES=YES;
RUN;
