***********************************************************************************************************************************************************************************;
*Program: 25.SAS_BICIndFullNumPredCstatTest_SimSce1Uncorr                                                                                                                          ;                                                               
*Purpose: Compute summary statistics for Number of predictors and C-statistic for Individual, Union, Intersect, and Full methods in Scenario 1 simulated testing uncorrelated data ;                                     
...
%let trainmod_ind=outdata.bicnewsim500uncorrby;
%let testdata=savedata.sim500uncorrTest;
...
data outdata.sim500indtestuncorr_c; set sim500indtest_c; run;
data outdata.sim500uniontestuncorr_c; set sim500uniontest_c; run;
data outdata.sim500intersecttestuncorr_c; set sim500intersecttest_c; run;
data outdata.sim500fulltestuncorr_c; set sim500fulltest_c; run;

*Summary stats of individual training simulated models on baBIC testing simulated data;
proc means data=outdata.sim500indtestuncorr_c stackodsoutput n mean clm stderr maxdec=4; 
 var C_adl C_iadl C_walk C_death;
 ods output summary=outdata.sim500indtestuncorr_cmeans(rename=(Mean=Mean_Sim_indtesting));
proc sort; by variable; run;

PROC EXPORT DATA= outdata.sim500indtestuncorr_cmeans
            OUTFILE= "path\sim500indtestuncorr_cmeans.csv" 
            DBMS=CSV REPLACE;
     PUTNAMES=YES;
RUN;

*Summary stats of union training simulated models on baBIC testing simulated data;
proc means data=outdata.sim500uniontestuncorr_c stackodsoutput n mean clm stderr maxdec=4; 
 var C_avg C_adl C_iadl C_walk C_death;
 ods output summary=outdata.sim500uniontestuncorr_cmeans(rename=(Mean=Mean_Sim_uniontesting));
proc sort; by variable; run;

PROC EXPORT DATA= outdata.sim500uniontestuncorr_cmeans
            OUTFILE= "path\sim500uniontestuncorr_cmeans.csv" 
            DBMS=CSV REPLACE;
     PUTNAMES=YES;
RUN;

*Summary stats of intersect training simulated models on baBIC testing simulated data;
proc means data=outdata.sim500intersecttestuncorr_c stackodsoutput n mean clm stderr maxdec=4; 
 var C_avg C_adl C_iadl C_walk C_death;
 ods output summary=outdata.sim500inttestuncorr_cmeans(rename=(Mean=Mean_Sim_inttesting));
proc sort; by variable; run;

PROC EXPORT DATA= outdata.sim500inttestuncorr_cmeans
            OUTFILE= "path\sim500inttestuncorr_cmeans.csv" 
            DBMS=CSV REPLACE;
     PUTNAMES=YES;
RUN;

*Summary stats of full training simulated models on baBIC testing simulated data;
proc means data=outdata.sim500fulltestuncorr_c stackodsoutput n mean clm stderr maxdec=4; 
 var C_avg C_adl C_iadl C_walk C_death;
 ods output summary=outdata.sim500fulltestuncorr_cmeans(rename=(Mean=Mean_Sim_fulltesting));
proc sort; by variable; run;

PROC EXPORT DATA= outdata.sim500fulltestuncorr_cmeans
            OUTFILE= "path\sim500fulltestuncorr_cmeans.csv" 
            DBMS=CSV REPLACE;
     PUTNAMES=YES;
RUN;
