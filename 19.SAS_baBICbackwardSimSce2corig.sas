***********************************************************************************************************************************************************************************;
*Program: 19.SAS_baBICbackwardSimSce2corig                                                                                                                                         ;                                                               
*Purpose: baBIC backward elimination for simulations of Scenario 2 with case-study censoring and training data                                                                     ;                                     
...
proc sort data=savedata.sim500trainindcorig out=simdata; by newid; run; 
...
%macro best_bic;
	...
	data _null_;
	   set savedata.bicsimind_Sce2corig (keep= sim &BICvars);
	   where sim=&l;
...
data savedata.bicsimbaBIC_Sce2corig; set BICsim; run;
...
data BICsim2;
 ...
 set savedata.bicsimbaBIC_Sce2corig(keep=sim VARINMODEL C_avg iauc_avg BIC_avg C_adl C_iadl C_walk C_death); 
...
data savedata.bicsimbaBIC_Sce2corig_2;
...
proc means data=bicsimbaBIC_Sce2corig_2  stackodsoutput n mean maxdec=4;
...
proc freq data=bicsimbaBIC_Sce2corig_2 ; tables sumbaBIC / nocum nofreq out=freq (keep=sumbaBIC percent rename=(percent=Percentage)); run;

proc print data=bicsimbaBIC_Sce2corig_2 (obs=10); var sim varsbaBIC VARINMODEL baBICy1-baBICy15;  where sumbaBIC=11; run;
...
proc freq data=bicsimbaBIC_Sce2corig_2; tables sumVarNoUnion / nocum nofreq out=freq2 (keep=sumVarNoUnion percent rename=(percent=Percentage)); run;
...
run;
ods rtf file="path\Report_BICSimSel&S.Sce2corig.rtf" startpage=no style=mystyle;
