***********************************************************************************************************************************************************************************;
*Program: 24.SAS_baBICbackwardSimSce3c25                                                                                                                                           ;                                                               
*Purpose: baBIC backward elimination for simulations of Scenario 3 with 25% censoring and training data                                                                            ;                                     
...
proc sort data=savedata.sim500trainfullc25 out=simdata; by newid; run; 
...
%macro best_bic;
	...
	data _null_;
	   set savedata.bicsimind_Sce3c25 (keep= sim &BICvars);
	   where sim=&l;
...
data savedata.bicsimbaBIC_Sce3c25; set BICsim; run;
...
data BICsim2;
 ...
 set savedata.bicsimbaBIC_Sce3c25(keep=sim VARINMODEL C_avg iauc_avg BIC_avg C_adl C_iadl C_walk C_death); 
...
data savedata.bicsimbaBIC_Sce3c25_2; 
...
proc contents data=savedata.bicsimbaBIC_Sce3c25_2; run;
...
proc means data=bicsimbaBIC_Sce3c25_2  stackodsoutput n mean maxdec=4;
...
proc freq data=bicsimbaBIC_Sce3c25_2 ; tables sumbaBIC / nocum nofreq out=freq (keep=sumbaBIC percent rename=(percent=Percentage)); run;

proc print data=bicsimbaBIC_Sce3c25_2 (obs=10); var sim varsbaBIC VARINMODEL baBICy1-baBICy15;  where sumbaBIC=11; run;

data means_freq;
 set means freq; 
 if sumbaBIC=3 then do; variable="model_3vars"; simulations=&S; end;
 else if sumbaBIC=4 then do; variable="model_4vars"; simulations=&S; end;
 else if sumbaBIC=5 then do; variable="model_5vars"; simulations=&S; end;
...
proc freq data=bicsimbaBIC_Sce3c25_2; tables sumVarNoUnion / nocum nofreq out=freq2 (keep=sumVarNoUnion percent rename=(percent=Percentage)); run;
...
run;
ods rtf file="path\Report_BICSimSel&S.Sce3c25.rtf" startpage=no style=mystyle;
