...
%let trainmodels=outtrain.bicsimbabic_Sce3c25;
...
proc means data=&trainmodels stackodsoutput n mean clm stderr p50 p25 p75 maxdec=4; 
 ...
 ods output summary=outtrain.bicsimbabic_Sce3c25_stats;
run;

PROC EXPORT DATA= outtrain.bicsimbabic_Sce3c25_stats
            OUTFILE= "path\bicsimbabic_Sce3c25_stats.csv" 
            DBMS=CSV REPLACE;
     PUTNAMES=YES;
RUN;
...
%macro cstat;
 ...
 /*For each simulated dataset define VARNAME as the variables selected in baBIC model of training data*/
  ...
  proc append base=c_baBICSce3c25 data=ctable force; run;
...
proc sort data=c_baBICSce3c25 out=outtest.c_baBICSce3c25; by sim; run;

*Summary stats of baBIC training simulated models on baBIC testing simulated data;
proc means data=outtest.c_baBICSce3c25 stackodsoutput n mean clm stderr maxdec=4; 
 var C_adl C_iadl C_walk C_death C_avg;
 ods output summary=outtest.c_baBICSce3c25_means (rename=(Mean=Mean_Sim_baBICtesting));
run;

PROC EXPORT DATA= outtest.c_baBICSce3c25_means
            OUTFILE= "path\c_baBICSce3c25_means.csv" 
            DBMS=CSV REPLACE;
     PUTNAMES=YES;
RUN;
