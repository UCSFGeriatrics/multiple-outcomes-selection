***********************************************************************************************************************************************************************************;
*Program: 12.SAS_BICbackwardIndOutcomeSimSce1Corr                                                                                                                                  ;                                                               
*Purpose: BIC backward elimination for individual outcomes for Scenario 1 simulated training correlated data. Obtain Union model for each simulation                               ;                                     
...
data savedata.BICnewsim&S.corrBy (drop=delims i variable allvars);
  set BICsim;
...
run;
/*proc print data=savedata.BICnewsim&S.corrBy (obs=3); var jointvars; run;*/

*Summary stats of simulated models by outcome;
proc means data=savedata.BICnewsim&S.corrBy stackodsoutput n mean clm stderr maxdec=4; 
 ...
 ods output summary=savedata.BICnewsim&S.corrBygralstats(rename=(Mean=MeanSimulationByOutcome));
proc sort; by variable; run;

PROC EXPORT DATA= savedata.BICnewsim&S.corrBygralstats
            OUTFILE= "path\BICnewsim&S.corrBygralstats.csv" 
            DBMS=CSV REPLACE;
     PUTNAMES=YES;
RUN;
