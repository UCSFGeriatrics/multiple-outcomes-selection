****************************************************************************************************************************************************************************************;
*Program: 6b.SAS_SimSce1CorrTest                                                                                                                                                        ;                                                               
*Purpose: Generate simulated testing datasets with correlated outcomes and predictors from baBIC model of HRS case-study data. Compute some statistics (Scenario 1, correlated outcomes);                                    
...
proc sort data=savedata.sim500corrTest out=simdata; by newid; run; 
proc contents data=savedata.sim500corrTest; run;
...
%let SIM=500; /*number of simulations*/
title;
ods rtf file='path\Results_Statssim500corrTestVsOriginal.rtf' startpage=no;
...
proc print data=savedata.pearsonsim500corrTest (firstobs=1 obs=10); run;
proc print data=savedata.pearsonsim500corrTest (firstobs=491 obs=502); run;

ods startpage=now;
ods text= "Simulated datasets (S=&SIM) Spearman Correlation";
proc print data=savedata.spearmansim500corrTest (firstobs=1 obs=10); run;
proc print data=savedata.spearmansim500corrTest (firstobs=491 obs=502); run;
