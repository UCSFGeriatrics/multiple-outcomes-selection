############### CumInc Plot: Scenario 2 correlated outcomes   #########################
...
dataADL <- read.csv('CumIncbaBICcorrby_adl.csv')
dataADL0<-subset(dataADL, sim==0)

dataIADL <- read.csv('CumIncbaBICcorrby_iadl.csv')
dataIADL0<-subset(dataIADL, sim==0)

dataWalk <- read.csv('CumIncbaBICcorrby_walk.csv')
dataWalk0<-subset(dataWalk, sim==0)

dataDeath <- read.csv('CumIncbaBICcorrby_death.csv')
dataDeath0<-subset(dataDeath, sim==0)

dataAll <- read.csv('CumIncbaBICcorrby_all.csv')
...
###########################color version##################################
png("CumIncSce2corr_R_color.png",width = 1800, height = 1500, res = 600)
ggplot() +
  # draw the original cif series with grey
...
pdf('CumIncSce2corr_R_color.pdf',width = 6, height = 5)
ggplot() +
  # draw the original cif series with grey
  geom_line(aes(time, cif, group = sim, color=outcome), data = dataAll,alpha=0.7,size=0.01) +
  ## added color for similation line (picked lighter color than the actual line)
