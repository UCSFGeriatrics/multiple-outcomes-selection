********************************************************************************************************************************************************************************************************;
*Program: 9b.SAS_SimSce2UncorrTest                                                                                                                                                                      ;                                                               
*Purpose: Generate simulated testing datasets with uncorrelated outcomes and predictors from Best Individual models of HRS case-study data. Compute some statistics (Scenario 2, uncorrelated outcomes) ;                                  
...
/*Create dataset/outcome only with covariates in the final individual ADL model for all Ids*/
data finaldata2adl; 
...
proc iml;
call randseed(12345);
u = j(22124000,1); /*allocate (22,124,000 x 1) vector */
...
*Select observations for testing set: 11,062,001-22,124,000 (training set 1-11,062,000);
data unidata2 (drop=obs);
 set unidata2;
 where 11062001<=obs<=22124000;
run;

data unidata2; set unidata2; obs=_N_;run;
...
/*Create dataset/outcome only with covariates in the final individual IADL model for all Ids*/
data finaldata2iadl; 
...
/*Create dataset/outcome only with covariates in the final individual DEATH model for all Ids*/
...
data savedata.sim500uncorrByTest;
...
proc sort data=savedata.sim500uncorrByTest out=simdata; by newid; run; 
...
PROC EXPORT DATA= savedata.PearsonSim&maxsim.&corr.By
            OUTFILE= "\PearsonSim&maxsim.&corr.By.csv" 
            DBMS=CSV REPLACE;
     PUTNAMES=YES;
RUN;
...
PROC EXPORT DATA= savedata.SpearmanSim&maxsim.&corr.By
            OUTFILE= "\SpearmanSim&maxsim.&corr.By.csv" 
            DBMS=CSV REPLACE;
     PUTNAMES=YES;
RUN;
...
%let SIM=500; /*number of simulations*/
title;
ods rtf file="path\Results_Statssim500uncorrByTestVsOriginal.rtf" startpage=no;
...
proc print data=savedata.pearsonsim500uncorrByTest (firstobs=1 obs=10); run;
proc print data=savedata.pearsonsim500uncorrByTest (firstobs=491 obs=502); run;

ods startpage=now;
ods text= "Simulated datasets (S=&SIM) Spearman Correlation";
proc print data=savedata.spearmansim500uncorrByTest (firstobs=1 obs=10); run;
proc print data=savedata.spearmansim500uncorrByTest (firstobs=491 obs=502); run;
