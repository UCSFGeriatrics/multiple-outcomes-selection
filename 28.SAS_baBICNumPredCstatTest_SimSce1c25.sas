***********************************************************************************************************************************************************************************;
*Program: 28.SAS_baBICNumPredCstatTest_SimSce1c25                                                                                                                                  ;                                                               
*Purpose: Compute summary statistics for number of predictors and C-statistic for baBIC method in simulations of Scenario 1 with 25% censoring and testing data                    ;                                     
...
%let trainmodels=outtrain.bicsimbabic_sce1c25;
...
proc means data=&trainmodels stackodsoutput n mean clm stderr p50 p25 p75 maxdec=4; 
 ...
 ods output summary=outtrain.bicsimbabic_sce1c25_stats;
run;

PROC EXPORT DATA= outtrain.bicsimbabic_sce1c25_stats
            OUTFILE= "path\bicsimbabic_sce1c25_stats.csv" 
            DBMS=CSV REPLACE;
     PUTNAMES=YES;
RUN;
...
%macro cstat;
 ...
 /*For each simulated dataset define VARNAME as the variables selected in baBIC model of training data*/
  ...
  proc append base=c_baBICsce1c25 data=ctable force; run;
...
proc sort data=c_baBICsce1c25 out=outtest.c_baBICsce1c25; by sim; run;

*Summary stats of baBIC training simulated models on baBIC testing simulated data;
proc means data=outtest.c_baBICsce1c25 stackodsoutput n mean clm stderr maxdec=4; 
 var C_adl C_iadl C_walk C_death C_avg;
 ods output summary=outtest.c_baBICsce1c25_means (rename=(Mean=Mean_Sim_baBICtesting));
run;

PROC EXPORT DATA= outtest.c_baBICsce1c25_means
            OUTFILE= "path\c_baBICsce1c25_means.csv" 
            DBMS=CSV REPLACE;
     PUTNAMES=YES;
RUN;
...
proc stdize data=outtest.c_baBICsce1c25 PctlMtd=ORD_STAT outstat=outtest.c_baBICsce1c25_percentiles pctlpts=2.5, 97.5;
...
data outtest.c_baBICsce1c25_percentiles;
 set outtest.c_baBICsce1c25_percentiles;
 where _type_ =: 'P';
...
proc print data=outtest.c_baBICsce1c25_percentiles noobs; run;
