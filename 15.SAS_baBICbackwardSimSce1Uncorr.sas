***********************************************************************************************************************************************************************************;
*Program: 15.SAS_baBICbackwardSimSce1Uncorr                                                                                                                                        ;                                                               
*Purpose: baBIC backward elimination for Scenario 1 simulated training uncorrelated data                                                                                           ;                                     
...
%macro best_bic;
	...
	data _null_;
	   set savedata.bicnewsim500uncorrby (keep= sim &BICvars);
	   where sim=&l;
...
data savedata.baBICsim&S.uncorrSce1; set BICsim; run;
...
data BICsim2;
 ...
 set savedata.baBICsim&S.uncorrSce1(keep=sim VARINMODEL C_avg iauc_avg BIC_avg C_adl C_iadl C_walk C_death); 
...
data savedata.bicnewsim&S.uncorr2plus; 
...
proc contents data=savedata.bicnewsim&S.uncorr2plus; run;
...
proc means data=savedata.bicnewsim&S.uncorr2plus stackodsoutput n mean clm stderr maxdec=4;
...
data gralstats;
 set savedata.bicnewsim&S.uncorr2plus  (keep=numVarsbaBIC C_meanfinal C_adlfinal C_iadlfinal C_walkfinal C_deathfinal IAUC_meanfinal BIC_meanfinal) end=last;
 if last=1;
run;
...
proc sort; by variable; run;
data savedata.BICnewsim&S.uncorrGralStatsPlus;
...
proc means data=savedata.bicnewsim&S.uncorr2plus  stackodsoutput n mean maxdec=4;
...
proc freq data=savedata.bicnewsim&S.uncorr2plus; tables sumbaBIC / nocum nofreq out=freq (keep=sumbaBIC percent rename=(percent=Percentage)); run;

proc print data=savedata.bicnewsim&S.uncorr2plus (obs=10); var sim varsbaBIC VARINMODEL baBICy1-baBICy15;  where sumbaBIC=11; run;
...
proc freq data=savedata.bicnewsim&S.uncorr2plus; tables sumVarNoUnion / nocum nofreq out=freq2 (keep=sumVarNoUnion percent rename=(percent=Percentage)); run;
...
run;
ods rtf file="path\Results_baBIC&S.uncorrStatsScenario1.rtf" startpage=no style=mystyle;
...
proc print data=savedata.BICnewsim&S.uncorrGralStatsPlus; run;
