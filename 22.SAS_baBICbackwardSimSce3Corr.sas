***********************************************************************************************************************************************************************************;
*Program: 22.SAS_baBICbackwardSimSce3Corr                                                                                                                                          ;                                                               
*Purpose: baBIC backward elimination for Scenario 3 simulated training correlated data                                                                                             ;                                     
...
%macro best_bic;
	...
	data _null_;
	   set savedata.BICnewsim500corrFull (keep= sim &BICvars);
	   where sim=&l;
...
data savedata.baBICsim&S.corrSce3; set BICsim; run;
...
data BICsim2;
 ...
 set savedata.baBICsim&S.corrSce3(keep=sim VARINMODEL C_avg iauc_avg BIC_avg C_adl C_iadl C_walk C_death); 
...
data savedata.bicnewsim&S.corr2full;
...
proc contents data=savedata.bicnewsim&S.corr2full; run;
...
proc means data=savedata.bicnewsim&S.corr2full stackodsoutput n mean clm stderr maxdec=4;
...
data gralstats;
 set savedata.bicnewsim&S.corr2full  (keep=numVarsbaBIC C_meanfinal C_adlfinal C_iadlfinal C_walkfinal C_deathfinal IAUC_meanfinal BIC_meanfinal) end=last;
 if last=1;
run;
...
proc sort; by variable; run;
data savedata.BICnewsim&S.corrGralStatsfull;
...
proc means data=savedata.bicnewsim&S.corr2full  stackodsoutput n mean maxdec=4;
...
proc freq data=savedata.bicnewsim&S.corr2full; tables sumbaBIC / nocum nofreq out=freq (keep=sumbaBIC percent rename=(percent=Percentage)); run;

proc print data=savedata.bicnewsim&S.corr2full (obs=10); var sim varsbaBIC VARINMODEL baBICy1-baBICy15;  where sumbaBIC=11; run;
...
proc freq data=savedata.bicnewsim&S.corr2full; tables sumVarNoUnion / nocum nofreq out=freq2 (keep=sumVarNoUnion percent rename=(percent=Percentage)); run;
...
run;
ods rtf file="path\Results_baBIC&S.corrStatsScenario3.rtf" startpage=no style=mystyle;
...
proc print data=savedata.BICnewsim&S.corrGralStatsfull; run;
