****************************************************************************************************************************************************************************************************;
*Program: 34.SAS_BICIndNumPredCstatTest_SimSce3c25                                                                                                                                                  ;                                                               
*Purpose: Compute summary statistics for number of predictors and C-statistic for Individual, Union, Intersection, and Full methods in simulations of Scenario 3 with 25% censoring and testing data;                                     
...
%let S=500;
%let trainmodels=outind.BICsimInd_Sce3c25;
%let testdata=outtest.sim500testFullc25;
...
PROC EXPORT DATA= &trainmodels._stats
            OUTFILE= "path\bicsimind_Sce3c25_stats.csv" 
            DBMS=CSV REPLACE;
     PUTNAMES=YES;
RUN;
...
%macro c_all(predictors=, common_pred=, model=);
 ...
 /* For each simulated dataset define VARNAME as in the individual/union/inters/full */
  ...
  proc append base=c_&model.Sce3c25 data=ctable force; run;
...
data outtest.c_indSce3c25; set c_indSce3c25; run;
data outtest.c_unionSce3c25; set c_unionSce3c25; run;
data outtest.c_intersSce3c25; set c_intersSce3c25; run;
data outtest.c_fullSce3c25; set c_fullSce3c25; run;

*Summary stats of individual training simulated models on baBIC testing simulated data;
proc means data=outtest.c_indSce3c25 stackodsoutput n mean stderr clm  maxdec=4; 
 var C_adl C_iadl C_walk C_death;
 ods output summary=outtest.c_indSce3c25_means(rename=(Mean=Mean_Sim_indtesting));
proc sort; by variable; run;

PROC EXPORT DATA= outtest.c_indSce3c25_means
            OUTFILE= "path\c_indSce3c25_means.csv" 
            DBMS=CSV REPLACE;
     PUTNAMES=YES;
RUN;

*Custom percentiles individual;
proc stdize data=outtest.c_indsce3c25 PctlMtd=ORD_STAT outstat=outtest.c_indsce3c25_percentiles pctlpts=2.5, 97.5;
 var C_adl C_iadl C_walk C_death ;
run;
/*Specify the PCTLMTD= option so that the algorithm uses the traditional "sort the data" algorithm for computing percentiles,
rather than a newer one-pass algorithm. Although the one-pass algorithm is very fast and well-suited for computing the median, 
...
data outtest.c_indsce3c25_percentiles;
 set outtest.c_indsce3c25_percentiles;
 where _type_ =: 'P';
...
proc print data=outtest.c_indsce3c25_percentiles noobs; run;



*Summary stats of union training simulated models on baBIC testing simulated data;
proc means data=outtest.c_unionSce3c25 stackodsoutput n mean stderr clm  maxdec=4; 
 var C_avg C_adl C_iadl C_walk C_death;
 ods output summary=outtest.c_unionSce3c25_means(rename=(Mean=Mean_Sim_uniontesting));
proc sort; by variable; run;

PROC EXPORT DATA= outtest.c_unionSce3c25_means
            OUTFILE= "path\c_unionSce3c25_means.csv" 
            DBMS=CSV REPLACE;
     PUTNAMES=YES;
RUN;

*Custom percentiles union;
proc stdize data=outtest.c_unionsce3c25 PctlMtd=ORD_STAT outstat=outtest.c_unionsce3c25_percentiles pctlpts=2.5, 97.5;
 var C_adl C_iadl C_walk C_death ;
run;
/*Specify the PCTLMTD= option so that the algorithm uses the traditional "sort the data" algorithm for computing percentiles,
rather than a newer one-pass algorithm. Although the one-pass algorithm is very fast and well-suited for computing the median, 
...
data outtest.c_unionsce3c25_percentiles;
 set outtest.c_unionsce3c25_percentiles;
 where _type_ =: 'P';
 C_adl=round(C_adl,0.01);
 C_iadl=round(C_iadl,0.01);
 C_walk=round(C_walk,0.01);
...
proc print data=outtest.c_unionsce3c25_percentiles noobs; run;


*Summary stats of inters training simulated models on baBIC testing simulated data;
proc means data=outtest.c_intersSce3c25 stackodsoutput n mean stderr clm  maxdec=4; 
 var C_avg C_adl C_iadl C_walk C_death;
 ods output summary=outtest.c_intersSce3c25_means(rename=(Mean=Mean_Sim_inttesting));
proc sort; by variable; run;

PROC EXPORT DATA= outtest.c_intersSce3c25_means
            OUTFILE= "path\c_intersSce3c25_means.csv" 
            DBMS=CSV REPLACE;
     PUTNAMES=YES;
RUN;
...
proc stdize data=outtest.c_interssce3c25 PctlMtd=ORD_STAT outstat=outtest.c_interssce3c25_percentiles pctlpts=2.5, 97.5;
 var C_adl C_iadl C_walk C_death ;
run;
/*Specify the PCTLMTD= option so that the algorithm uses the traditional "sort the data" algorithm for computing percentiles,
rather than a newer one-pass algorithm. Although the one-pass algorithm is very fast and well-suited for computing the median, 
...
data outtest.c_interssce3c25_percentiles;
 set outtest.c_interssce3c25_percentiles;
 where _type_ =: 'P';
 C_adl=round(C_adl,0.01);
 C_iadl=round(C_iadl,0.01);
 C_walk=round(C_walk,0.01);
...
proc print data=outtest.c_interssce3c25_percentiles noobs; run;


*Summary stats of full training simulated models on baBIC testing simulated data;
proc means data=outtest.c_fullSce3c25 stackodsoutput n mean stderr clm  maxdec=4; 
 var C_avg C_adl C_iadl C_walk C_death;
 ods output summary=outtest.c_fullSce3c25_means(rename=(Mean=Mean_Sim_fulltesting));
proc sort; by variable; run;

PROC EXPORT DATA= outtest.c_fullSce3c25_means
            OUTFILE= "path\c_fullSce3c25_means.csv" 
            DBMS=CSV REPLACE;
     PUTNAMES=YES;
RUN;

*Custom percentiles full;
proc stdize data=outtest.c_fullsce3c25 PctlMtd=ORD_STAT outstat=outtest.c_fullsce3c25_percentiles pctlpts=2.5, 97.5;
 var C_adl C_iadl C_walk C_death ;
run;
/*Specify the PCTLMTD= option so that the algorithm uses the traditional "sort the data" algorithm for computing percentiles,
rather than a newer one-pass algorithm. Although the one-pass algorithm is very fast and well-suited for computing the median, 
...
data outtest.c_fullsce3c25_percentiles;
 set outtest.c_fullsce3c25_percentiles;
 where _type_ =: 'P';
 C_adl=round(C_adl,0.01);
 C_iadl=round(C_iadl,0.01);
 C_walk=round(C_walk,0.01);
...
proc print data=outtest.c_fullsce3c25_percentiles noobs; run;
