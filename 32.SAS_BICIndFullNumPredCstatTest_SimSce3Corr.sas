***********************************************************************************************************************************************************************************;
*Program: 32.SAS_BICIndFullNumPredCstatTest_SimSce3Corr                                                                                                                            ;                                                               
*Purpose: Compute summary statistics for Number of predictors and C-statistic for Individual, Union, Intersect, and Full methods in Scenario 3 simulated testing correlated data   ;                                     
...
%let trainmod_ind=outdata.bicnewsim500corrFull;
%let testdata=savedata.sim500corrFullTest;
...
/*
======MONITORING: 2020-09-21, 15:31======
======MONITORING: 2020-09-21, 15:43======
======MONITORING: 2020-09-21, 15:57======
======MONITORING: 2020-09-21, 16:08======
======MONITORING: 2020-09-21, 16:25======
*/

*Save permanent datasets;
data outdata.sim500indtestby_c; set sim500indtest_c; run;
data outdata.sim500uniontestby_c; set sim500uniontest_c; run;
data outdata.sim500intersecttestby_c; set sim500intersecttest_c; run;
data outdata.sim500fulltestby_c; set sim500fulltest_c; run;

*Summary stats of individual training simulated models on baBIC testing simulated data;
proc means data=outdata.sim500indtestby_c stackodsoutput n mean clm stderr maxdec=4; 
 var C_adl C_iadl C_walk C_death;
 ods output summary=outdata.sim500indtestby_cmeans(rename=(Mean=Mean_Sim_indtesting));
proc sort; by variable; run;

PROC EXPORT DATA= outdata.sim500indtestby_cmeans
            OUTFILE= "path\sim500indtestby_cmeans.csv" 
            DBMS=CSV REPLACE;
     PUTNAMES=YES;
RUN;

*Summary stats of union training simulated models on baBIC testing simulated data;
proc means data=outdata.sim500uniontestby_c stackodsoutput n mean clm stderr maxdec=4; 
 var C_avg C_adl C_iadl C_walk C_death;
 ods output summary=outdata.sim500uniontestby_cmeans(rename=(Mean=Mean_Sim_uniontesting));
proc sort; by variable; run;

PROC EXPORT DATA= outdata.sim500uniontestby_cmeans
            OUTFILE= "path\sim500uniontestby_cmeans.csv" 
            DBMS=CSV REPLACE;
     PUTNAMES=YES;
RUN;

*Summary stats of intersect training simulated models on baBIC testing simulated data;
proc means data=outdata.sim500intersecttestby_c stackodsoutput n mean clm stderr maxdec=4; 
 var C_avg C_adl C_iadl C_walk C_death;
 ods output summary=outdata.sim500inttestby_cmeans(rename=(Mean=Mean_Sim_inttesting));
proc sort; by variable; run;

PROC EXPORT DATA= outdata.sim500inttestby_cmeans
            OUTFILE= "path\sim500inttestby_cmeans.csv" 
            DBMS=CSV REPLACE;
     PUTNAMES=YES;
RUN;

*Summary stats of full training simulated models on baBIC testing simulated data;
proc means data=outdata.sim500fulltestby_c stackodsoutput n mean clm stderr maxdec=4; 
 var C_avg C_adl C_iadl C_walk C_death;
 ods output summary=outdata.sim500fulltestby_cmeans(rename=(Mean=Mean_Sim_fulltesting));
proc sort; by variable; run;

PROC EXPORT DATA= outdata.sim500fulltestby_cmeans
            OUTFILE= "path\sim500fulltestby_cmeans.csv" 
            DBMS=CSV REPLACE;
     PUTNAMES=YES;
RUN;
