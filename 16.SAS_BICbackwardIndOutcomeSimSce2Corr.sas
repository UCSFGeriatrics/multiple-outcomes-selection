***********************************************************************************************************************************************************************************;
*Program: 16.SAS_BICbackwardIndOutcomeSimSce2Corr                                                                                                                                  ;                                                               
*Purpose: BIC backward elimination for individual outcomes for Scenario 2 simulated training correlated data. Obtain Union model for each simulation                               ;                                     
...
data savedata.BICsim_&label; set BICsim_&label; proc sort; by sim; run;
...
data BICsim;
   merge savedata.BICsim_adl savedata.BICsim_iadl savedata.BICsim_walk savedata.BICsim_death;
   by sim;
run;

data savedata.BICsim&S.corrBy (drop=delims i variable allvars);
  set BICsim;
...
run;
/*proc print data=savedata.BICsim&S.corrBy (obs=3); var jointvars; run;*/

*Summary stats of simulated models by outcome;
proc means data=savedata.BICsim&S.corrBy stackodsoutput n mean clm stderr maxdec=4; 
 ...
 ods output summary=savedata.bicsim&S.corrbygralstats(rename=(Mean=MeanSimulationByOutcome));
proc sort; by variable; run;

PROC EXPORT DATA= savedata.bicsim&S.corrbygralstats
            OUTFILE= "path\bicsim&S.corrbygralstats.csv" 
            DBMS=CSV REPLACE;
     PUTNAMES=YES;
RUN;
