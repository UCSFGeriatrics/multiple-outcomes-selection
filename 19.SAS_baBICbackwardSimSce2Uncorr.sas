***********************************************************************************************************************************************************************************;
*Program: 19.SAS_baBICbackwardSimSce2Uncorr                                                                                                                                        ;                                                               
*Purpose: baBICbackward elimination for Scenario 2 simulated training uncorrelated data                                                                                            ;                                     
...
proc sort data=savedata.sim500uncorrbyTrain out=simdata; by newid; run; 
...
%macro best_bic;
	...
	data _null_;
	   set savedata.bicsim500uncorrby (keep= sim &BICvars);
	   where sim=&l;
...
data savedata.baBICsim&S.uncorrSce2; set BICsim; run;
...
data BICsim2;
 ...
 set savedata.baBICsim&S.uncorrSce2(keep=sim VARINMODEL C_avg iauc_avg BIC_avg C_adl C_iadl C_walk C_death);
...
data savedata.bicnewsim&S.uncorrbyoutcome2plus;
...
proc contents data=savedata.bicnewsim&S.uncorrbyoutcome2plus; run;
...
proc means data=savedata.bicnewsim&S.uncorrbyoutcome2plus stackodsoutput n mean clm stderr maxdec=4;
...
data gralstats;
 set savedata.bicnewsim&S.uncorrbyoutcome2plus  (keep=numVarsbaBIC C_meanfinal C_adlfinal C_iadlfinal C_walkfinal C_deathfinal IAUC_meanfinal BIC_meanfinal) end=last;
 if last=1;
run;
...
proc sort; by variable; run;
data savedata.BICnewsim&S.uncorrByOutStatsPlus;
...
proc means data=savedata.bicnewsim&S.uncorrbyoutcome2plus  stackodsoutput n mean maxdec=4;
...
proc freq data=savedata.bicnewsim&S.uncorrbyoutcome2plus; tables sumbaBIC / nocum nofreq out=freq (keep=sumbaBIC percent rename=(percent=Percentage)); run;

proc print data=savedata.bicnewsim&S.uncorrbyoutcome2plus (obs=10); var sim varsbaBIC VARINMODEL baBICy1-baBICy15;  where sumbaBIC=11; run;
...
proc freq data=savedata.bicnewsim&S.uncorrbyoutcome2plus; tables sumVarNoUnion / nocum nofreq out=freq2 (keep=sumVarNoUnion percent rename=(percent=Percentage)); run;
...
run;
ods rtf file="path\Report_baBIC&S.uncorrStatsScenario2.rtf" startpage=no style=mystyle;
...
proc print data=savedata.BICnewsim&S.uncorrByOutStatsPlus; run;
...
ods startpage=now;
ods text="Percentage of incorrect inclusion/variable NOT-selected in Union model using &S simulated datasets with ununcorrelated outcomes";
proc print data=means_freq2 noobs; var Variable Simulations Percentage; run;
ods text="Mean of Percentage of incorrect inclusion/variable NOT-selected in Union model using &S simulated datasets with ununcorrelated outcomes";
proc means data=meansNoUnion n mean; var Mean; run;
ods rtf close;
