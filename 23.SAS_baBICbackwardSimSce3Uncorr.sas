***********************************************************************************************************************************************************************************;
*Program: 23.SAS_baBICbackwardSimSce3Uncorr                                                                                                                                        ;                                                               
*Purpose: baBIC backward elimination for Scenario 3 simulated training uncorrelated data                                                                                           ;                                     
...
proc sort data=savedata.sim500uncorrFullTrain out=simdata; by newid; run; 
...
%let NUMPREDfullONE=38; /*number of predictors defined in &NUMPRED (e.g.37) full one*/
...
%macro best_bic;
	...
	data _null_;
	   set savedata.BICnewsim500uncorrFull (keep= sim &BICvars);
	   where sim=&l;
...
%macro DeleteOneVar;
    %do j=1 %to %eval(&NUMPREDfullONE-&i);
...
data savedata.baBICsim&S.uncorrSce3; set BICsim; run;
...
data BICsim2;
 ...
 set savedata.baBICsim&S.uncorrSce3(keep=sim VARINMODEL C_avg iauc_avg BIC_avg C_adl C_iadl C_walk C_death); 
...
data savedata.bicnewsim&S.uncorr2full; 
...
proc contents data=savedata.bicnewsim&S.uncorr2full; run;
...
proc means data=savedata.bicnewsim&S.uncorr2full stackodsoutput n mean clm stderr maxdec=4;
...
data gralstats;
 set savedata.bicnewsim&S.uncorr2full  (keep=numVarsbaBIC C_meanfinal C_adlfinal C_iadlfinal C_walkfinal C_deathfinal IAUC_meanfinal BIC_meanfinal) end=last;
 if last=1;
run;
...
proc sort; by variable; run;
data savedata.BICnewsim&S.uncorrGralStatsfull;
...
proc means data=savedata.bicnewsim&S.uncorr2full  stackodsoutput n mean maxdec=4;
...
proc freq data=savedata.bicnewsim&S.uncorr2full; tables sumbaBIC / nocum nofreq out=freq (keep=sumbaBIC percent rename=(percent=Percentage)); run;

proc print data=savedata.bicnewsim&S.uncorr2full (obs=10); var sim varsbaBIC VARINMODEL baBICy1-baBICy15;  where sumbaBIC=11; run;
...
*Percentage of uncorrect inclusion of variables in baBIC model;
proc means data=savedata.bicnewsim&S.uncorr2full  stackodsoutput n mean maxdec=4;
...
*Percentage of INuncorrECT inclusion of variables in union model using S simulated datasets;
proc means data=savedata.bicnewsim&S.uncorr2full stackodsoutput n mean maxdec=4;
...
proc freq data=savedata.bicnewsim&S.uncorr2full; tables sumVarNoUnion / nocum nofreq out=freq2 (keep=sumVarNoUnion percent rename=(percent=Percentage)); run;
...
run;
ods rtf file="path\Results_baBIC&S.uncorrStatsScenario3.rtf" startpage=no style=mystyle;
ods text= "Summary statistics for best model in original data vs best models using &S simulated datasets with ununcorrelated outcomes";
proc print data=savedata.BICnewsim&S.uncorrGralStatsfull; run;

ods startpage=now;
ods text="Percentage of each variable selected in the final model using &S simulated datasets with ununcorrelated outcomes";
proc print data=means_freq noobs; var Variable Simulations Percentage; run;
ods text="Mean of Percentage of uncorrect Inclusion for selected variables (i.e. gender and age were forced in) in the final model using &S simulated datasets";
proc means data=meansbaBIC n mean; var Mean; where Variable not in ('baBICy1', 'baBICy2'); run;
 
ods startpage=now;
ods text="Percentage of INuncorrECT inclusion/variable NOT-selected in Union model using &S simulated datasets with ununcorrelated outcomes";
proc print data=means_freq2 noobs; var Variable Simulations Percentage; run;
ods text="Mean of Percentage of INuncorrECT inclusion/variable NOT-selected in Union model using &S simulated datasets with ununcorrelated outcomes";
proc means data=meansNoUnion n mean; var Mean; run;
ods rtf close;
