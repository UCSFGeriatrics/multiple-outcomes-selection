***********************************************************************************************************************************************************************************;
*Program: 27.SAS_baBICNumPredCstatTest_SimSce1corig                                                                                                                                ;                                                               
*Purpose: Compute summary statistics for number of predictors and C-statistic for baBIC method in simulations of Scenario 1 with case-study censoring and testing data             ;                                     
...
%let trainmodels=outtrain.bicsimbabic_sce1corig;
...
proc means data=&trainmodels stackodsoutput n mean clm stderr p50 p25 p75 maxdec=4; 
 ...
 ods output summary=outtrain.bicsimbabic_sce1corig_stats;
run;

PROC EXPORT DATA= outtrain.bicsimbabic_sce1corig_stats
            OUTFILE= "path\bicsimbabic_sce1corig_stats.csv" 
            DBMS=CSV REPLACE;
     PUTNAMES=YES;
RUN;
...
%macro cstat;
 ...
 /*For each simulated dataset define VARNAME as the variables selected in baBIC model of training data*/
  ...
  proc append base=c_baBICsce1corig data=ctable force; run;
...
proc sort data=c_baBICsce1corig out=outtest.c_baBICsce1corig; by sim; run;

*Summary stats of baBIC training simulated models on baBIC testing simulated data;
proc means data=outtest.c_baBICsce1corig stackodsoutput n mean clm stderr maxdec=4; 
 var C_adl C_iadl C_walk C_death C_avg;
 ods output summary=outtest.c_baBICsce1corig_means (rename=(Mean=Mean_Sim_baBICtesting));
run;

PROC EXPORT DATA= outtest.c_baBICsce1corig_means
            OUTFILE= "path\c_baBICsce1corig_means.csv" 
            DBMS=CSV REPLACE;
     PUTNAMES=YES;
RUN;

*Custom percentiles baBIC;
proc stdize data=outtest.c_baBICsce1corig PctlMtd=ORD_STAT outstat=outtest.c_baBICsce1corig_percentiles pctlpts=2.5, 97.5;
...
data outtest.c_baBICsce1corig_percentiles;
 set outtest.c_baBICsce1corig_percentiles;
 where _type_ =: 'P';
...
proc print data=outtest.c_baBICsce1corig_percentiles noobs; run;
