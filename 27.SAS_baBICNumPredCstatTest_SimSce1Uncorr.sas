***********************************************************************************************************************************************************************************;
*Program: 27.SAS_baBICNumPredCstatTest_SimSce1Uncorr                                                                                                                               ;                                                               
*Purpose: Compute summary statistics for Number of predictors and C-statistic for baBIC method in Scenario 1 simulated testing uncorrelated data                                   ;                                     
...
%let trainmod=outdata.baBICsim500uncorrSce1;
...
%cstat;
%PUT ======MONITORING: %SYSFUNC(DATE(),YYMMDD10.), %LEFT(%SYSFUNC(TIME(),HHMM8.))======;
proc sort data=ctablesim out=outdata.sim&S.baBICuncorrtest_c; by sim; run;

PROC EXPORT DATA=outdata.sim&S.baBICuncorrtest_c
            OUTFILE= "path\sim&S.baBICuncorrtest_c.csv" 
            DBMS=CSV REPLACE;
     PUTNAMES=YES;
RUN;

*Summary stats of baBIC training simulated models on baBIC testing simulated data;
proc means data=outdata.sim&S.baBICuncorrtest_c stackodsoutput n mean clm stderr maxdec=4; 
 var C_adl C_iadl C_walk C_death C_avg;
 ods output summary=outdata.sim&S.baBICuncorrtest_cmeans(rename=(Mean=Mean_Sim_baBICuncorrtesting));
proc sort; by variable; run;

PROC EXPORT DATA= outdata.sim&S.baBICuncorrtest_cmeans
            OUTFILE= "path\sim&S.baBICuncorrtest_cmeans.csv" 
            DBMS=CSV REPLACE;
     PUTNAMES=YES;
RUN;
