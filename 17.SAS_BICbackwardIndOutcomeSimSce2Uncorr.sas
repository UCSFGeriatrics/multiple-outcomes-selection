***********************************************************************************************************************************************************************************;
*Program: 17.SAS_BICbackwardIndOutcomeSimSce2Uncorr                                                                                                                                ;                                                               
*Purpose: BIC backward elimination for individual outcomes for Scenario 2 simulated training uncorrelated data. Obtain Union model                                                 ;                                     
...
data savedata.BICsimuncorr_&label; set BICsim_&label; proc sort; by sim; run;
...
data BICsim;
   merge savedata.BICsimuncorr_adl savedata.BICsimuncorr_iadl savedata.BICsimuncorr_walk savedata.BICsimuncorr_death;
   by sim;
run;

data savedata.BICsim&S.uncorrBy (drop=delims i variable allvars);
  set BICsim;
...
run;
/*proc print data=savedata.BICsim&S.uncorrBy (obs=3); var jointvars; run;*/

*Summary stats of simulated models by outcome;
proc means data=savedata.BICsim&S.uncorrBy stackodsoutput n mean clm stderr maxdec=4; 
 ...
 ods output summary=savedata.bicsim&S.uncorrbygralstats(rename=(Mean=MeanSimulationByOutcome));
proc sort; by variable; run;

PROC EXPORT DATA= savedata.bicsim&S.uncorrbygralstats
            OUTFILE= "path\bicsim&S.uncorrbygralstats.csv" 
            DBMS=CSV REPLACE;
     PUTNAMES=YES;
RUN;
