***********************************************************************************************************************************************************************************;
*Program: 21.SAS_BICbackwardIndOutcomeSimSce3Uncorr                                                                                                                                ;                                                               
*Purpose: BIC backward elimination for individual outcomes for Scenario 3 simulated training uncorrelated data. Obtain Union model for each simulation                             ;                                     
...
3-) Create a dataset with one line uncorresponding to the best model for each individual outcome and each simulation
*/
...
data savedata.BICsim_uncorr_&label; set BICsim_&label; proc sort; by sim; run;
...
data BICsim;
   merge savedata.BICsim_uncorr_adl savedata.BICsim_uncorr_iadl savedata.BICsim_uncorr_walk savedata.BICsim_uncorr_death;
   by sim;
run;

data savedata.BICnewsim&S.uncorrFull (drop=delims i variable allvars);
  set BICsim;
...
run;
/*proc print data=savedata.BICnewsim&S.uncorrFull (obs=3); var jointvars; run;*/

*Summary stats of simulated models by outcome;
proc means data=savedata.BICnewsim&S.uncorrFull stackodsoutput n mean clm stderr maxdec=4; 
 ...
 ods output summary=savedata.BICnewsim&S.uncorrFullgralstats(rename=(Mean=MeanSimulationByOutcome));
proc sort; by variable; run;

PROC EXPORT DATA= savedata.BICnewsim&S.uncorrFullgralstats
            OUTFILE= "path\BICnewsim&S.uncorrFullgralstats.csv" 
            DBMS=CSV REPLACE;
     PUTNAMES=YES;
RUN;
