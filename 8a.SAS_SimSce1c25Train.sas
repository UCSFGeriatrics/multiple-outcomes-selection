***********************************************************************************************************************************************************************************;
*Program: 8a.SAS_SimSce1c25Train                                                                                                                                                   ;
*Purpose: Import training simulated datasets under Scenario 1 (baBIC method) with 25% censoring from R and obtain some gral statistics                                             ;                                     
...
%macro impdata (inputdata=, rep=, outputdata=);
	%do i=1 %to &rep;
		PROC IMPORT OUT= temp
		            ...
		            DBMS=CSV REPLACE;
		     GETNAMES=YES;
			 GUESSINGROWS=752216; /*to avoid variable failed to be truncated*/
		     DATAROW=2; 
		RUN;
...
%impdata(inputdata=sim10trainbaBICc25_20210109_, rep=50, outputdata=outtrain.sim500trainbaBICc25);
...
title;
ods rtf file='\path\Results_StatsSimTrainbaBICc25VsOriginal.rtf' startpage=no;
