*******************************************************************************************************************************************************************************************;
*Program: 11b.SAS_SimSce3UncorrTest                                                                                                                                                        ;                                                               
*Purpose: Generate simulated testing datasets with uncorrelated outcomes and predictors from Full model of HRS case-study data. Compute some statistics (Scenario 3, uncorrelated outcomes);                                     
...
%macro SimGen;
 ...
 %let VARNAME=dAGE SEX ALCOHOL ARTHRITIS CANCER COGDLRC3G COGIMRC3G DIABETES DRIVE EDU EXERCISE EYE2G
			     FALL HEARAID HEARING HEARTFAILURE HYPERTENSION INCONTINENCE LALONE LUNG MSTAT OTHERARM
	                     OTHERCHAIR OTHERCLIM3G OTHERDIME OTHERLIFT OTHERPUSH OTHERSIT OTHERSTOOP OTHERWALK PAIN CESDALL
			     qBMI qFAGE qMAGE SHLT SMOKING STROKE VOLUNTEER; /*name of variables in Fullmodel*/
...
data savedata.sim500uncorrFullTest;
...
proc sort data=savedata.sim500uncorrFullTest out=simdata; by newid; run; 
...
%let SIM=500; /*number of simulations*/
title;
ods rtf file="path\Results_Statssim500uncorrFullTestVsOriginal.rtf" startpage=no;
...
proc print data=savedata.pearsonsim500uncorrFullTest (firstobs=1 obs=10); run; 
proc print data=savedata.pearsonsim500uncorrFullTest (firstobs=491 obs=502); run;

ods startpage=now;
ods text= "Simulated datasets (S=&SIM) Spearman Correlation";
proc print data=savedata.spearmansim500uncorrFullTest (firstobs=1 obs=10); run;
proc print data=savedata.spearmansim500uncorrFullTest (firstobs=491 obs=502); run;
