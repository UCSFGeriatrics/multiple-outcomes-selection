***********************************************************************************************************************************************************************************;
*Program: 31.SAS_baBICNumPredCstatTest_SimSce2Uncorr                                                                                                                               ;                                                               
*Purpose: Compute summary statistics for Number of predictors and C-statistic for baBIC method in Scenario 2 simulated testing uncorrelated data                                   ;                                     
...
%let trainmod=outdata.baBICsim500uncorrSce2;
...
proc sort data=ctablesim out=outdata.sim&S.baBICtestbyuncorr_c; by sim; run;

PROC EXPORT DATA=outdata.sim&S.baBICtestbyuncorr_c
            OUTFILE= "path\bsim&S.baBICtestby_c.csv" 
            DBMS=CSV REPLACE;
     PUTNAMES=YES;
RUN;

*Summary stats of baBIC training simulated models on baBIC testing simulated data;
proc means data=outdata.sim&S.baBICtestbyuncorr_c stackodsoutput n mean clm stderr maxdec=4; 
 var C_adl C_iadl C_walk C_death C_avg;
 ods output summary=outdata.sim&S.baBICtestbyuncorr_cmeans(rename=(Mean=Mean_Sim_baBICtesting));
proc sort; by variable; run;

PROC EXPORT DATA= outdata.sim&S.baBICtestbyuncorr_cmeans
            OUTFILE= "path\sim&S.baBICtestbyuncorr_cmeans.csv" 
            DBMS=CSV REPLACE;
     PUTNAMES=YES;
RUN;
